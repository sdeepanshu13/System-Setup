...
| File | Purpose |
|------|---------|
| `Windows/bootstrap-dev.sh` | Main entry point — runs everything in order |
| `Windows/restore.ps1` | Installs all software via `winget import` |
| `Windows/winget-packages.json` | List of 49 winget packages (dev tools, languages, browsers, etc.) |
| `Windows/vscode-extensions.txt` | 41 VS Code extensions to restore |
| `Windows/zshrc-template` | Optimized `.zshrc` for Git Bash with Powerlevel10k |
...
```powershell
.\Windows\restore.ps1 -WhatIfMode
```

**Shell setup only** (if software is already installed):
```bash
# Comment out Phase 0 in bootstrap-dev.sh, or just run the zsh/font steps manually
./bootstrap-dev.sh
```

## Customizing

- **Add/remove winget packages**: Edit `Windows/winget-packages.json`
- **Add/remove VS Code extensions**: Edit `Windows/vscode-extensions.txt` (one extension ID per line)
- **Change shell config**: Edit `Windows/zshrc-template`

## Updating the Package List

To re-export your current system's packages:
```powershell
winget export -o Windows/winget-packages.json --accept-source-agreements
```

To re-export VS Code extensions:
```bash
code --list-extensions > Windows/vscode-extensions.txt
```